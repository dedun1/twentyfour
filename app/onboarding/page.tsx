...
export default function OnboardingPage() {
  ...
  const handleStartConsultation = async () => {
    ...
    const inProgress = (listData.sessions || []).find((s: any) => s.status === 'in_progress' && s.has_messages);
    if (inProgress?.id) {
          ...
          {view === 'choice' && resumeSessionId ? (
              <Card className="mt-12">
                <CardContent className="p-6 text-center space-y-4">
                  ...
                  <p className="text-sm text-muted-foreground">You have an in-progress consultation waiting.</p>
                  <div className="flex gap-3 justify-center flex-wrap">
                    <Button onClick={() => setView('chat')} className="bg-amber-500 text-black hover:bg-amber-400">
                      Continue {resumeSessionId ? 'previous chat' : 'your last consultation'}
                    </Button>
                    <Button
                      variant="outline"
                      onClick={() => {
            ...
            <ConsultationChat
              ...
              onComplete={(payload: ApiCompleteResponse) => {
                ...
                if (sid) router.push(`/onboarding/recommendations?session=${encodeURIComponent(sid)}`);
              }}
            />
          )}
        </div>

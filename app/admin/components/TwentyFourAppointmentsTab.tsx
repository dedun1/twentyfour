            ...
            {monthlyData.every((item) => item.revenue === 0) ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={monthlyData}>
                  ...
                  <XAxis dataKey="month" tick={{ fill: 'var(--muted-foreground)', fontSize: 11 }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fill: 'var(--muted-foreground)', fontSize: 11 }} axisLine={false} tickLine={false} allowDecimals={false} />
                  <Tooltip contentStyle={tooltipStyle} formatter={(value) => [formatEgp(Number(value || 0)), 'Revenue']} />
                  ...
                  {recentAppointments.map((appointment) => {
                    ...
                    return (
                      <TableRow key={appointment.id} className={isCancelled ? 'bg-red-500/5' : ''}>
                        ...
                        <TableCell className={isCancelled ? 'line-through decoration-red-500/70 text-red-500' : ''}>{formatEgp(Number(appointment.cost || 0))}</TableCell>
                      </TableRow>
                    );
                  })}
                </TableBody>

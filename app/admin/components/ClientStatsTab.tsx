...
export function ClientStatsTab() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const [{ data: clientRows, error: clientError }, { data: appointmentRows, error: appointmentError }, adminIds] = await Promise.all([
        supabase.from('clients').select('id, business_name').order('business_name', { ascending: true }),
        supabase
          .from('appointments')
      ...
      if (clientError || appointmentError) {
        toast.error(clientError?.message || appointmentError?.message || 'Failed to load client stats');
        setLoading(false);
        return;
      }

      const nonAdminClients = ((clientRows || []) as ClientRow[]).filter((client) => !adminIds.includes(client.id));
      const nonAdminAppointments = ((appointmentRows || []) as AppointmentRow[]).filter((appointment) => !adminIds.includes(appointment.client_id || ''));
      setAdminClientIds(adminIds);
      setClients(nonAdminClients);
      setAppointments(nonAdminAppointments);
      setLoading(false);
    };
  ...
  const selectedAppointments = useMemo(() => {
    if (selectedClientId === 'all') return appointments.filter((appointment) => !adminClientIds.includes(appointment.client_id || ''));
    return appointments.filter((appointment) => appointment.client_id === selectedClientId);
  }, [adminClientIds, appointments, selectedClientId]);
  const selectedClientLabel = selectedClientId === 'all'
    ? 'All Clients (Aggregated)'
    : (clients.find((client) => client.id === selectedClientId)?.business_name || 'Select client');
  ...
  const monthlyData = useMemo(() => {
    ...
    for (const appointment of selectedAppointments) {
      ...
      if (appointment.status !== 'cancelled') {
        bucket.revenue += Number(appointment.cost || 0);
      }
    }
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader className="space-y-2">
          ...
          <Select value={selectedClientId} onValueChange={(value) => setSelectedClientId(value ?? 'all')}>
            <SelectTrigger className="w-full md:w-[360px]">
              <SelectValue placeholder="Select client">{selectedClientLabel}</SelectValue>
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All Clients (Aggregated)</SelectItem>
              {clients.map((client) => (
